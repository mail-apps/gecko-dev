...
TraceRecorder::isGlobal(jsval* p) const
{
    return ((size_t(p - globalObj->fslots) < JS_INITIAL_NSLOTS) ||
            (size_t(p - globalObj->dslots) < (MAX_GLOBAL_SLOTS - JS_INITIAL_NSLOTS)));
}
               ...
               VMSideExit** innermostNestedGuardp)
{
    ...
    memset(global, 0xCD, MAX_GLOBAL_SLOTS*sizeof(double));
    size_t canary_idx = JS_MIN(MAX_GLOBAL_SLOTS, STOBJ_NSLOTS(globalObj));
#endif

    debug_only(*(uint64*)&global[canary_idx]= 0xdeadbeefdeadbeefLL;)
...
LeaveTree(InterpState& state, VMSideExit* lr)
{
    ...
    JSObject* globalObj = JS_GetGlobalForObject(cx, cx->fp->scopeChain);
    size_t canary_idx = JS_MIN(MAX_GLOBAL_SLOTS, STOBJ_NSLOTS(globalObj));
    JS_ASSERT(*(uint64*)&global[canary_idx] == 0xdeadbeefdeadbeefLL);
#endif
...
js_InitJIT(JSTraceMonitor *tm)
{
    ...
    if (!tm->nativeData) {
        tm->nativeData = malloc(sizeof(InterpState) + sizeof(double) * (MAX_GLOBAL_SLOTS + 1));
